...
def WordsOfLenghth(lenght, pos):
    glossary = loadGlossary()
    return list(
        filter(lambda x: len(x["lemma"]) == lenght and x["pos"] == pos, glossary)
    )


def mkConlluToken(word, entry, head=0, deprel=None, start=0, ident=1, deps=None):
    mapping = {"ADP": "case", "SCONJ": "mark", "VERB": "root", "PUNCT": "punct"}
    end = start + len(word)
    feats = {}
    token = Token()
    token["id"] = ident
...
def handleNCont(upos, feats):
    if feats["Rel"] == "NCont":
        if upos == "VERB":
            feats.update({"Number": "Sing", "Person": "3", "VerbForm": "Fin"})
        elif upos == "NOUN":
            feats.update({"Number[psor]": "Sing", "Person[psor]": "3"})
        elif upos == "ADP":
            feats.update({"Number[grnd]": "Sing", "Person[grnd]": "3"})
...
def TreebankSentence(text="", pref="", textid=0, index=0, sentid=0):
    ...
    if len(sents) == 5:
        ppText(
            [yrl, sents[3], sents[2], sents[1], sents[4]], pref, textid, index, sentid
        )
    else:
...
def mkDict(text, pref="MooreFP1994", textid=0, sentid=1):
    ...
    parts = [
        re.sub(r"\s-\s", "", part).strip() for part in re.split(r"[)(]", groups[1])
    ]
    dic["source"] = parts[1]
