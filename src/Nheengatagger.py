...
from src.BuildDictionary import extract_feats, loadLexicon, guesser
...
def propernames(namelist=cp.NAMES):
    dic = {}
...
def includePunctuation(
    punctuation=cp.PUNCTUATION, ellipis=cp.ELLIPSIS, elip=cp.ELIP, dashes=cp.DASHES
):
    punctdict = {}
    tagSet = set()
    tagSet.add("PUNCT")
...
def buildDictionary(infile=cp.DATA_PATH.joinpath("lexicon.json")):
    tagger = {}
...
# DICTIONARY = buildDictionary(cp.LEXICON_PATH)
# MWE = extractMWEs(DICTIONARY)


def splitPunctuation(token, punctuation=cp.PUNCTUATION):
    tokenlist = []
    c = len(token)
    if c == 1:
...
def tokenize(
    sentence,
    mwe=extractMWEs(buildDictionary(cp.LEXICON_PATH)),
    mwe_sep=" ",
    ellipis=cp.ELLIPSIS,
    xxxx=cp.XXXX,
):
    sentence = sentence.replace(cp.ELLIPSIS, cp.XXXX)
    ...
    while i < len(tokenList) - 1:
        ...
        if firstchar in cp.PUNCTUATION:
            newList.append(firstchar)
            thisToken = thisToken[1:]
        if lastchar in cp.PUNCTUATION:
            ...
            if wordList:
                ...
                if nextToken in wordList:
                    ...
                    if nextToken[-1] in cp.PUNCTUATION and nextToken[:-1] in wordList:
...
def tagWord(token, tagger=None):
    if not tagger:
        tagger = buildDictionary(cp.LEXICON_PATH)
    tags = tagger.get(tuple(token.split()))
    if not tags:
        parselist = guesser(token, loadLexicon(cp.LEXICON_PATH))
        tags = set()
        if parselist:
...
def tagSentence(sentence, tagger=buildDictionary(cp.LEXICON_PATH), unknown="???"):
    tagged = []
...
def parseWord(word, lexicon=None, infile=cp.LEXICON_PATH):
...
def getparselist(word, lexicon=None, infile=cp.LEXICON_PATH):
    word = word.lower()
    if lexicon:
        lexicon = lexicon
    else:
